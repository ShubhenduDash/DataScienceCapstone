...
```{r message=FALSE}
# Get the file list
listOfFiles <- dir("HC Corpora", recursive = TRUE, full.names = TRUE)
...
Next, a couple of sample lines of Twitter files are shown:
 ...
 - ***de_DE.twitter.txt***:
```{r}
connnectionBlogsFile <- file("HC Corpora/de_DE/de_DE.twitter.txt", "r")
readLines(connnectionBlogsFile, 3)
close(connnectionBlogsFile)
```
 - ***en_US.twitter.txt***:
```{r}
connnectionBlogsFile <- file("HC Corpora/en_US/en_US.twitter.txt", "r")
readLines(connnectionBlogsFile, 3)
close(connnectionBlogsFile)
```
...
tweets <- readLines('HC Corpora/en_US/en_US.twitter.txt', encoding = 'UTF-8', skipNul = TRUE)
tweets <- iconv(tweets, to = "ASCII", sub="")

blogs <- readLines('HC Corpora/en_US/en_US.blogs.txt', encoding = 'UTF-8', skipNul = TRUE)

newsFileConnection <- file('HC Corpora/en_US/en_US.news.txt', encoding = 'UTF-8', open = 'rb')
...
swearWordsFileUrl <- 'Terms-to-Block.csv'
...
This document has been generated using [***R Mardown***](http://rmarkdown.rstudio.com/). Its ***.Rmd*** source code that can be found at: <https://github.com/ShubhenduDash/DataScienceCapstone>.
